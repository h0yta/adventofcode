...
create_test_data_file() {
  ...
  local data_file1="./test/data/${year}/day${day}-1"
  local data_file2="./test/data/${year}/day${day}-2"
  ...
  if [[ ! -f "${data_file1}" ]]; then
    log "Creating new test data file ${data_file1}"
    touch ${data_file1}
  else
    log_error "File already exists ${data_file1}"
  fi

  if [[ ! -f "${data_file2}" ]]; then
    log "Creating new test data file ${data_file2}"
    touch ${data_file2}
  else
    log_error "File already exists ${data_file2}"
  fi
}
